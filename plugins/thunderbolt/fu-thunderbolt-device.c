...
{
	const gchar *version_raw = NULL;
	g_auto(GStrv) split = NULL;
	g_autofree gchar *version = NULL;

	for (guint i = 0; i < 50; i++) {
		version_raw = fu_udev_device_get_sysfs_attr (FU_UDEV_DEVICE (self),
							     "nvm_version",
							     NULL);
		if (version_raw != NULL)
			break;
...
fu_thunderbolt_device_check_safe_mode (FuThunderboltDevice *self)
{
	...
	safe_path = g_build_path ("/", self->devpath, "nvm_version", NULL);
...
{
	FuThunderboltDevice *self = FU_THUNDERBOLT_DEVICE (device);
	const gchar *parent_name = fu_udev_device_get_parent_name (FU_UDEV_DEVICE (self));
...
/*	TODO: inhibit the idle sleep of the daemon
...
*/
	...
	gen = fu_thunderbolt_device_get_attr_uint16 (self, "generation", &error_gen);
	if (gen == 0)
...
static gboolean
fu_plugin_thunderbolt_trigger_update (FuThunderboltDevice	*self,
				      GError			**error)
{
				      ...
				      GError **error)
{
	...
	if (!fu_plugin_thunderbolt_trigger_update (self, error)) {
